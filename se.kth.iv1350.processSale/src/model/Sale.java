...
public class Sale {
	...
	/*
	 * calculate the difference between @param totalPaid and @param totalToPay and
	 * return change
	 */
	public double calculateChange(double totalPaid) {
		notifyRevenueObserver();
		return totalPaid - this.totalToPay;
	}
	...
	public void addToList(ItemDTO item) {
		...
		this.listWithRegisteredItems.add(
				new RegisteredItemDTO(item.getName(), item.getArticleNumber(), item.getPrice(), item.getVatRate(), 1));
	}

	private void notifyRevenueObserver() {
		for (TotalRevenueObserver obs : totalRevenueObserver) {
			obs.newTotalIncome(getTotalToPay());
		}
	}

	public void addObservers(List<TotalRevenueObserver> totalRevenueObserver) {
		this.totalRevenueObserver.addAll(totalRevenueObserver);
	}
